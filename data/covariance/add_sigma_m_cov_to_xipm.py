...
filename=FolderNameInputs+'kids/multiplicative_bias/m_cov_r_1.00.ascii'
file  = open(filename)
...
# 
				...
				filename=FolderNameInputs+'../input_for_xipm_sigma_m_covariance/chain/output_test_A/shear_xi_plus_binned/bin_'+str(z2+1)+'_'+str(z1+1)+'.txt'
				file  = open(filename)
				xip_z1_z2 = np.loadtxt(file)
# 
				filename=FolderNameInputs+'../input_for_xipm_sigma_m_covariance/chain/output_test_A/shear_xi_plus_binned/bin_'+str(z4+1)+'_'+str(z3+1)+'.txt'
				file  = open(filename)
				xip_z3_z4 = np.loadtxt(file)
# 
				filename=FolderNameInputs+'../input_for_xipm_sigma_m_covariance/chain/output_test_A/shear_xi_minus_binned/bin_'+str(z2+1)+'_'+str(z1+1)+'.txt'
				file  = open(filename)
				xim_z1_z2 = np.loadtxt(file)
# 
				filename=FolderNameInputs+'../input_for_xipm_sigma_m_covariance/chain/output_test_A/shear_xi_minus_binned/bin_'+str(z4+1)+'_'+str(z3+1)+'.txt'
				file  = open(filename)
...
filename = FolderNameInputs + '../covariance/inputs/thps_cov_kids1000_xipm_apr24/thps_cov_kids1000_xipm_apr6_matrix_with_sigma_m.dat'
